...
- If --mapping-csv not provided, will auto-use input_csv/name_map_technology.csv
  if that file exists (relative to this script)
...
- mapping table is idempotent: never dropped; UPSERT only.
...
Usage:
  ...
  python create_db_2026.py --db isp.sqlite --mapping-csv input_csv/name_map_technology.csv
"""
...
-- mapping must be idempotent (never dropped)
...
def upsert_mapping(conn: sqlite3.Connection, df_map: pd.DataFrame, dry_run: bool) -> None:
    required = ["mapping_key", "mapped_group", "mapped_name"]
    missing = [c for c in required if c not in df_map.columns]
    if missing:
        raise KeyError(f"mapping-csv missing columns {missing}. Present={list(df_map.columns)}")

    rows = []
    for _, r in df_map.iterrows():
        rows.append((norm_str(r["mapping_key"]), r.get("mapped_group"), r.get("mapped_name")))

    if dry_run:
        logging.info("[dry-run] would UPSERT %d mapping rows", len(rows))
        return

    conn.executemany(
        """
        INSERT INTO mapping (mapping_key, mapped_group, mapped_name, updated_utc)
        VALUES (?, ?, ?, datetime('now'))
        ON CONFLICT(mapping_key) DO UPDATE SET
            mapped_group=excluded.mapped_group,
...
def melt_and_split(parsed: Dict[str, Any]) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    annual_long = annual.rename(columns={"source_col_str": "year"})[["year", "value", "source_row", "source_col", "id_json"]]
    nonannual_long = nonannual.rename(columns={"source_col_str": "label"})[["label", "value", "source_row", "source_col", "id_json"]]
...
def iter_input_files(patterns: Optional[Sequence[str]], project_root: Path) -> List[Path]:
    ...
    for pat in patterns:
        p = Path(pat)
        if p.is_absolute():
            # treat as filesystem glob from drive root
            out.extend([Path(x) for x in p.parent.glob(p.name)])
        else:
            # relative to project_root
            # support ** by using rglob when pattern contains **
            full_pat = (project_root / pat)
            if "**" in pat:
                # crude but effective: split at ** and rglob the suffix
                # e.g. hidden_data/**/*.xlsx -> base=hidden_data, suffix=*.xlsx
                parts = pat.split("**")
                base = (project_root / parts[0].rstrip("/\\"))
                suffix = parts[-1].lstrip("/\\") or "*"
                if not base.exists():
                    continue
...
def default_mapping_csv(project_root: Path) -> Optional[Path]:
    p = project_root / "input_csv" / "name_map_technology.csv"
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    ap.add_argument(
        "--mapping-csv",
        default=None,
        help="Optional mapping CSV. If omitted, uses input_csv/name_map_technology.csv if present.",
    )
    ...
    # Inputs default to hidden_data/**/*.xlsx
    input_files = iter_input_files(args.inputs, project_root)

    # Mapping default to input_csv/name_map_technology.csv if present
    mapping_csv = Path(args.mapping_csv) if args.mapping_csv else default_mapping_csv(project_root)
    ...
    try:
        ...
        if mapping_csv is not None:
            ...
            upsert_mapping(conn, df_map, dry_run=args.dry_run)
            logging.info("Mapping loaded from: %s", mapping_csv)
