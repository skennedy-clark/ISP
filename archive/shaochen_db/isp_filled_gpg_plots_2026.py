...
# ---------------------------------------------------------------------------
# Technology name normalisation (trust input_csv/name_map_technology.csv)
# ---------------------------------------------------------------------------
TECH_MAP_CSV = os.path.join(os.getcwd(), "input_csv", "name_map_technology.csv")
tech_map_df = pd.read_csv(TECH_MAP_CSV)
...
def normalise_tech(s: pd.Series) -> pd.Series:
    s = s.astype(str).str.strip()
    return s.map(tech_map).fillna(s)

capacity["Technology_std"]   = normalise_tech(capacity["Technology"])
generation["Technology_std"] = normalise_tech(generation["Technology"])

# ---------------------------------------------------------------------------
# GPG gas technologies (AFTER mapping)
# ---------------------------------------------------------------------------
gpg_tech_std = [
    "Mid-merit Gas",
    "Peaking Gas+Liquids",
...
capacity_gpg   = capacity[capacity["Technology_std"].isin(gpg_tech_std)].copy()
generation_gpg = generation[generation["Technology_std"].isin(gpg_tech_std)].copy()
...
METRICS = [
    {
        ...
        "outfile":   "Gas_filled_Capacity.pdf",
    },
    {
        "name":      "UF",
        "df":        util_factor_gpg,
    ...
    },
    {
        ...
        "outfile":   "Gas_filled_Generation.pdf",
    },
]
